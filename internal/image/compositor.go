...
// Product is resized to fit frame dimensions.
func (c *Compositor) Composite(product, frame image.Image, bgColor string) *CompositeResult {
	...
	// Resize product to fill frame dimensions (scale up if smaller)
	resizedProduct := c.service.ResizeToFill(product, width, height)

	// Draw product (fills entire canvas)
	draw.Draw(canvas, canvas.Bounds(), resizedProduct, image.Point{}, draw.Over)
